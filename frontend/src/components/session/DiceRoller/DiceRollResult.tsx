    ...
    <div className="mt-4 p-3 bg-accent/20 border border-accent rounded-lg">
      ...
      {result.type === 'attack' && (
        <>
          ...
          <div className="text-xs text-text-secondary mt-2 text-center">
            Ataque: [{result.dice?.join(', ') || (Array.isArray(result.details?.rolls) ? ((result.details.rolls as unknown as Array<{ die?: number; value?: number } | number>).map((r) => typeof r === 'number' ? r : (r.value || r.die || 0)).join(', ')) : '')}] + {(result as any).bonus || result.skillBonus || 0} = {result.total}
